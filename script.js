// Script animasi mengetik
const fullText = "Izzati\nNurazizi";
const el = document.getElementById("typed-name");
let i = 0;
function render(t) { el.innerHTML = t.replace('\n','<br>'); }
function typeChar() {
  if (i <= fullText.length) { render(fullText.slice(0,i++)); setTimeout(typeChar, 95); }
  else { setTimeout(deleteChar, 2500); }
}
function deleteChar() {
  if (i > 0) { render(fullText.slice(0,--i)); setTimeout(deleteChar, 60); }
  else { setTimeout(typeChar, 500); }
}
